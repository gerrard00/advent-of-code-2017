...
func fn(in string) int {
	...
	for _, row := range rows {
		...
		for _, digitString := range digitsAsStrings {
			currentDigit, _ := strconv.Atoi(digitString)
			// fmt.Println(digitString, currentDigit)

			if lowest == -1 || currentDigit < lowest {
				lowest = currentDigit
			}
			if highest == -1 || currentDigit > highest {
		...
		// fmt.Println(highest, lowest, result)
	}
	return result
}
