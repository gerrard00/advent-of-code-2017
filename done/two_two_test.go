...
var splitExp = regexp.MustCompile("\\s+")

func fn(in string) int {
	var result int

	rows := strings.Split(in, "\n")

	for _, row := range rows {
		digitsAsStrings := splitExp.Split(row, -1)
		digits := make([]int, len(digitsAsStrings))
		rowValue := -1

		for pos, digitString := range digitsAsStrings {
			digits[pos], _ = strconv.Atoi(digitString)
			// fmt.Println(digitString, digits[pos])
		}

		// fmt.Println(digits)
		for posNum, possibleNumerator := range digits {
			for posDiv, possibleDivisor := range digits {
				// fmt.Println(">>>>>>>>>>>>>>.")
				// fmt.Println(posNum, posDiv)
				// fmt.Println(possibleNumerator, possibleDivisor)
				if posNum != posDiv && possibleNumerator%possibleDivisor == 0 {
					result += possibleNumerator / possibleDivisor
					break
				}
...
const realInput = `1224	926	1380	688	845	109	118	88	1275	1306	91	796	102	1361	27	995
