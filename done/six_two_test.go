...
func fn(banks []int) int {
	previousConfigurations := make(map[string]int)
	...
	for {
		...
		if previousConfigurations[currentConfiguration] > 0 {
			fmt.Printf("Found duplicate configuration %v %v\n", previousConfigurations[currentConfiguration], cycles)
			return cycles - previousConfigurations[currentConfiguration]
		}
