...
func fn(numberOfPrograms int, numberOfIteraitons int, commandString string) string {
	p := newPrograms(numberOfPrograms)
	commands := getCommands(commandString)

	iterationResultCache := make(map[string]int)

	// p.print()

	for iterationIndex := 0; iterationIndex < numberOfIteraitons; iterationIndex++ {
		...
		currentIterationResult := p.String()

		if indexOfCycleStart, hasKey := iterationResultCache[currentIterationResult]; hasKey {
			period := iterationIndex - indexOfCycleStart

			// our cache is zero based, subtract one
			indexOfFinalResult := (numberOfIteraitons % period) - 1
			if indexOfFinalResult == -1 {
				// our last result before this was the one to keep
				indexOfFinalResult = iterationIndex - 1
			}

			for cachedIterationResult, indexOfCachedResult := range iterationResultCache {
				if indexOfCachedResult == indexOfFinalResult {
					return cachedIterationResult
				}
			}
		}

		iterationResultCache[currentIterationResult] = iterationIndex
	}
...
var testsOne = []testpair{
	...
	// {16, 1, getRealInput(), "ionlbkfeajgdmphc"},
	{16, 1000000000, getRealInput(), "fdnphiegakolcmjb"},
}

func runTest(t *testing.T, testInputs []testpair) {
	for _, pair := range testInputs {
		v := fn(pair.numberOfPrograms, pair.numberOfIterations, pair.commands)
